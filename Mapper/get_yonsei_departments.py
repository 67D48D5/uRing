...
def crawl_single_campus(url, campus_name):
    ...
    try:
        ...
        college_pattern = re.compile(r'([가-힣]+대학)$')  # Only those ending with "대학"
                ...
                if not any(d['name'] == text for d in current_college['departments']):
                    ...
                    if dept_url:
                        if dept_url == "NOT_FOUND":
                            print("Homepage link found but URL is `NOT_FOUND` at department:", text)
                        
                        subdomain_match = re.match(r'https?://([^.]+)\.yonsei\.ac\.kr', dept_url)
                        if subdomain_match:
